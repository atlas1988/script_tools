...
int parse_packet_80211(u_char* buf, u_int32 len){
	...
	if (ta != NULL) {
	   ...
	   rc = sqlite3_exec(db, sql, sqlit_callback, 0, &zErrMsg);
	   if( rc != SQLITE_OK ){
	      printf("SQL error: %s\n", zErrMsg);
	...
	if (ra != NULL) {
		...
		printf("RA	 %s\n", ether_sprintf(ra));
			 /* Create SQL statement */
		sprintf(sql,"SELECT * from OUI WHERE MAC='%02X-%02X-%02X'",ra[0],ra[1],ra[2]);
	   //sql = "SELECT * from OUI WHERE MAC='00-0A-4D'";
		printf("sql = %s \n",sql);
	...
	if (bssid != NULL) {
		...
		printf("BSSID %s\n", ether_sprintf(bssid));
			 /* Create SQL statement */
		sprintf(sql,"SELECT * from OUI WHERE MAC='%02X-%02X-%02X'",bssid[0],bssid[1],bssid[2]);
	   //sql = "SELECT * from OUI WHERE MAC='00-0A-4D'";
		printf("sql = %s \n",sql);
